#!/usr/bin/env python3
"""
Improved RAG System V2 - Uses improved OCR and embeddings for better Q&A performance
"""
...
class ImprovedRAGSystemV2:
    def __init__(self):
        # Try to use improved embeddings first, fallback to original
        self.storage_paths = [
            "./qdrant_storage_v2",  # New improved embeddings
            "./qdrant_storage"      # Original embeddings as fallback
        ]
        self.collection_names = [
            "pdf_embeddings_v2",    # New collection
            "pdf_embeddings"        # Original collection
        ]
        
        # Use your actual API key here
        self.api_key = "AIzaSyBqXqXqXqXqXqXqXqXqXqXqXqXqXqXqXqX"
        
        # Retrieval settings optimized for improved embeddings
        self.primary_limit = 50
        self.top_k_to_use = 15
        self.score_threshold_primary = 0.3  # Higher threshold for better quality
        self.min_text_length = 50
        self.max_text_per_doc = 800
    ...
    def _init_components(self):
        ...
        try:
            # Try to connect to improved embeddings first
            for i, (storage_path, collection_name) in enumerate(zip(self.storage_paths, self.collection_names)):
                try:
                    ...
                    logger.info(f"✅ Connected to {storage_path} with {collection_info.points_count} embeddings")
                    break
                except Exception as e:
                    if i == len(self.storage_paths) - 1:  # Last attempt
                        raise e
                    logger.warning(f"Failed to connect to {storage_path}, trying fallback...")
                    continue
    ...
    def expand_query(self, question: str) -> List[str]:
        ...
        variants.extend(keywords[:3])
        
        # Try Gemini for translation/keywords
        try:
            translate_prompt = (
                "Translate the following question to English and provide 3 short keyword variations.\n"
        ...
        try:
            ...
            for q in queries:
                ...
                for r in results:
                    ...
                    boosted = float(r.score or 0.0) + 0.05 * (overlap ** 0.5)
    ...
    def generate_answer(self, question: str, context: str) -> str:
        ...
        try:
            ...
            prompt = (
                "You are an expert assistant analyzing documents with improved OCR quality. "
...
def main():
    ...
    try:
        print("🚀 Initializing Improved RAG System V2...")
        rag = ImprovedRAGSystemV2()
